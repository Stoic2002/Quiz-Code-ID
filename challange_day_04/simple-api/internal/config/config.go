...
func Load() *Config {
	...
	if err := viper.ReadInConfig(); err != nil {
		log.Printf("Warning: Error reading config file %s: %v, using defaults", configPath, err)
	} else {
...
func setDefaults() {
	...
	viper.SetDefault("STORAGE.ALLOWED_TYPES.IMAGE", []string{"image/jpeg", "image/jpg", "image/png", "image/gif"})
	viper.SetDefault("STORAGE.ALLOWED_TYPES.DOCUMENT", []string{"application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"})
